...
const SEVERITY_CONFIG: Record<AlertSeverity, { bg: string; border: string; iconBg: string; badgeBg: string; badgeText: string; badgeLabel: string; icon: string; iconColor: string }> = {
  critical: {
    bg: '#FFF1F2',
    border: '#FECDD3',
    iconBg: '#FEE2E2',
...
export default function AdminAlertsScreen() {
  ...
  return (
    <SafeAreaView className="flex-1 bg-slate-50" edges={['top', 'left', 'right']}>
      {/* Header */}
      <View className="px-4 py-3.5 bg-white border-b border-slate-100 flex-row items-center justify-between">
        <View>
          <Text className="text-base font-bold text-slate-800">Platform Alerts</Text>
          <Text className="text-[10px] text-slate-400 mt-0.5">Live compliance and system issue feed</Text>
        </View>
      ...
      >
        ...
        <View className="flex-row space-x-3 mb-5">
          {[
            { label: 'Critical', count: criticalCount, color: '#EF4444', bg: '#FEE2E2', border: '#FECACA' },
            { label: 'Warnings', count: warningCount, color: '#F59E0B', bg: '#FEF3C7', border: '#FDE68A' },
            { label: 'Info', count: infoCount, color: '#0EA5E9', bg: '#E0F2FE', border: '#BAE6FD' },
          ].map((s) => (
            <View
              key={s.label}
              style={{ flex: 1, backgroundColor: s.bg, borderWidth: 1, borderColor: s.border, borderRadius: 16, padding: 12, alignItems: 'center' }}
            >
              <Text style={{ fontSize: 22, fontWeight: 'bold', color: s.color }}>{isLoading ? '—' : s.count}</Text>
              <Text style={{ fontSize: 9, color: s.color, fontWeight: '700', textTransform: 'uppercase', letterSpacing: 0.6, marginTop: 2 }}>{s.label}</Text>
            </View>
          ))}
        </View>
              ...
              return (
                <TouchableOpacity
                  ...
                  style={{
                    ...
                    borderWidth: 1, borderColor: isActive ? '#1E3A8A' : '#E2E8F0',
                    flexDirection: 'row', alignItems: 'center',
                  }}
                >
                  <Text style={{ fontSize: 11, fontWeight: '700', color: isActive ? '#ffffff' : '#64748B' }}>{f.label}</Text>
                  {f.count > 0 && (
                    <View style={{
                      marginLeft: 6, backgroundColor: isActive ? 'rgba(255,255,255,0.25)' : '#F1F5F9',
                      borderRadius: 8, paddingHorizontal: 5, paddingVertical: 1,
                    }}>
                      <Text style={{ fontSize: 9, fontWeight: '800', color: isActive ? '#ffffff' : '#475569' }}>{f.count}</Text>
                    </View>
                  )}
                </TouchableOpacity>
              );
        ...
        {isLoading && !refreshing ? (
          ...
          <View className="flex-1 justify-center items-center px-6 py-16 bg-white rounded-3xl border border-slate-100 shadow-sm">
            ...
            <Text className="text-base font-bold text-slate-800">All Clear</Text>
          ...
          <View className="space-y-3">
            {filteredAlerts.map((alert) => {
              ...
              return (
                <View
                  key={alert.id}
                  style={{ backgroundColor: cfg.bg, borderWidth: 1, borderColor: cfg.border, borderRadius: 20, padding: 16 }}
                >
                  {/* Top row */}
                  <View style={{ flexDirection: 'row', alignItems: 'flex-start', marginBottom: 10 }}>
                    <View style={{ width: 36, height: 36, borderRadius: 18, backgroundColor: cfg.iconBg, alignItems: 'center', justifyContent: 'center', marginRight: 10, flexShrink: 0 }}>
                    ...
                    <View style={{ flex: 1 }}>
                      <View style={{ flexDirection: 'row', alignItems: 'center', marginBottom: 2, flexWrap: 'wrap', gap: 6 }}>
                        <View style={{ backgroundColor: cfg.badgeBg, borderRadius: 6, paddingHorizontal: 7, paddingVertical: 2 }}>
                          <Text style={{ fontSize: 8, fontWeight: '800', color: cfg.badgeText, textTransform: 'uppercase', letterSpacing: 0.7 }}>{alert.category}</Text>
                        </View>
                        {alert.timestamp && (
                          <Text style={{ fontSize: 9, color: '#94A3B8' }}>
                            {new Date(alert.timestamp).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}
                          </Text>
                        )}
                      </View>
                      <Text style={{ fontSize: 13, fontWeight: '700', color: '#1E293B' }}>{alert.title}</Text>
                      <Text style={{ fontSize: 10, color: '#64748B', marginTop: 2 }}>{alert.subtitle}</Text>
                    </View>
                  </View>

                  {/* Detail box */}
                  {alert.detail && (
                    <View style={{ backgroundColor: 'rgba(0,0,0,0.04)', borderRadius: 12, padding: 10, marginBottom: 10 }}>
                      <Text style={{ fontSize: 10, color: '#475569', lineHeight: 16 }}>{alert.detail}</Text>
                    </View>
                  )}
                  ...
                  {alert.route && (
                    <TouchableOpacity
                      ...
                      activeOpacity={0.7}
                      style={{
                        borderWidth: 1, borderColor: cfg.border, borderRadius: 12,
                        paddingVertical: 8, alignItems: 'center', flexDirection: 'row', justifyContent: 'center',
                        backgroundColor: 'rgba(255,255,255,0.6)',
                      }}
                    >
                      <Text style={{ fontSize: 11, fontWeight: '700', color: '#1E3A8A', marginRight: 4 }}>{alert.routeLabel}</Text>
